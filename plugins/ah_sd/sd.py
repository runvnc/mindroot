...
async def sdxl_text_to_image(prompt, negative_prompt='', model_id=None, from_huggingface=True, count=1, save_to="imgs/" + random_img_fname(), w=1024, h=1024, steps=20, cfg=8):
    if model_id is None:
...
async def sd_text_to_image(prompt, negative_prompt='', model_id=None, from_huggingface=True, count=1, save_to="imgs/" + random_img_fname(), w=512, h=512, steps=20, cfg=8):
    if model_id is None:
        model_id = current_model
    if not from_huggingface:
