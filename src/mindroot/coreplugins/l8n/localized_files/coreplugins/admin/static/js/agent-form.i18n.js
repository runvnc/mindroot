...
class AgentForm extends BaseEl {
  ...
  async fetchAgents() {
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_agents__');
      this.agents = await response.json();
    } catch (error) {
      showNotification('error', `__TRANSLATE_error_loading_agents__: ${error.message}`);
    }
  }
  ...
  async handleAgentChange(event) {
    ...
    if (agentName) {
      try {
        ...
        if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_agent__');
        ...
        showNotification('error', `__TRANSLATE_error_loading_agent__: ${error.message}`);
      }
    } else {
      this.agent = null;
      this.newAgent = false;
  ...
  async fetchMissingCommands() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_missing_commands__');
      this.missingCommands = await response.json();
      console.log('Fetched missing commands:', this.missingCommands);
      this.requestUpdate();
    } catch (error) {
      this.missingCommands = {};
      console.log(`__TRANSLATE_error_fetching_missing_commands__: ${error.message}`);
    }
  }
  ...
  async fetchServiceModels() {
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_service_models__');
      ...
      showNotification('error', `__TRANSLATE_error_loading_service_models__: ${error.message}`);
    }
  }
  ...
  async fetchPersonas() {
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_personas__');
      this.personas = await response.json();
      console.log('Fetched personas:', this.personas);
      console.log('Current agent persona:', this.agent?.persona);
    } catch (error) {
      showNotification('error', `__TRANSLATE_error_loading_personas__: ${error.message}`);
    }
  }
  ...
  async fetchPlugins() {
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_plugins__');
      ...
      showNotification('error', `__TRANSLATE_error_loading_plugins__: ${error.message}`);
    }
  }
  ...
  async checkRecommendedPlugins() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_check_recommended__');
      const result = await response.json();
      ...
      console.log(`__TRANSLATE_error_checking_recommended_plugins__: ${error.message}`);
      this.pendingPlugins = [];
    }
  }
  ...
  async installRecommendedPlugins() {
    ...
    const result = await response.json();
    showNotification('success', '__TRANSLATE_info_recommended_plugins_installed__');
    this.checkRecommendedPlugins();
  }
  ...
  async fetchCommands() {
    try {
      ...
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_fetch_commands__');
      const data = await response.json();
      this.commands = this.organizeCommands(data);
    } catch (error) {
      showNotification('error', `__TRANSLATE_error_loading_commands__: ${error.message}`);
    }
  }
  ...
  validateForm() {
    ...
    if (!currentName?.trim()) {
      showNotification('error', '__TRANSLATE_validation_name_required__');
      return false;
    }
    if (!currentPersona?.trim()) {
      showNotification('error', '__TRANSLATE_validation_persona_required__');
      return false;
    }
    if (!currentInstructions?.trim()) {
      showNotification('error', '__TRANSLATE_validation_instructions_required__');
      return false;
    }
    return true;
  }
  ...
  async handleSubmit(event) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('__TRANSLATE_error_failed_to_save_agent__');
      }
      ...
      showNotification('success', `__TRANSLATE_info_agent_saved_successfully__ ${this.agent.name}`);
      ...
      showNotification('error', `__TRANSLATE_error_saving_agent__: ${error.message}`);
    } finally {
      this.loading = false;
    }
  }
  ...
  renderRequiredPlugins() {
    ...
    if (!this.plugins || this.plugins.length === 0) {
      ...
      return html`<div>__TRANSLATE_loading_plugins__</div>`;
    }
    
    return html`
      <div class=\"commands-category\">
        <h4>__TRANSLATE_header_recommended_plugins__</h4>
  ...
  renderServiceModels() {
    if (this.serviceModels === undefined || Object.keys(this.serviceModels).length === 0) {
      return html`<div>__TRANSLATE_loading_service_models__</div>`;
    }
    ...
    return html`
      <div class=\"commands-category\">
      <h4>__TRANSLATE_header_service_models__</h4>
      <div class=\"commands-grid\">
  ...
  renderPreferredProviders() {
    ...
    return html`
      <div class=\"commands-category\">
        <h4>__TRANSLATE_header_preferred_providers__</h4>
        <div class=\"commands-grid\">
          ${this.plugins.map(plugin => {
  ...
  renderPendingPlugins() {
    ...
    return html`
      <div class=\"form-group commands-section\">
        <div class=\"commands-category\">
          <h4>__TRANSLATE_header_pending_recommended_plugins__</h4>
          <p>__TRANSLATE_info_pending_plugins_description__</p>
          <ul>
            ${this.pendingPlugins.map(plugin => html`<li>${plugin}</li>`)}
          </ul>
          <button class=\"btn\" @click=${this.installRecommendedPlugins}>
            __TRANSLATE_button_install_recommended_plugins__
          </button>
        </div>
      </div>
    `;
  ...
  async handleExportAgent() {
    if (!this.agent?.name) {
      showNotification('error', '__TRANSLATE_error_no_agent_for_export__');
      return;
    }

    try {
      const response = await fetch(`/agents/local/${this.agent.name}/export`);
      if (!response.ok) throw new Error('__TRANSLATE_error_failed_to_export_agent__');
      ...
      showNotification('success', `__TRANSLATE_info_agent_exported_successfully__ ${this.agent.name}`);
    } catch (error) {
      showNotification('error', `__TRANSLATE_error_exporting_agent__: ${error.message}`);
    }
  }
  ...
  async handleImportAgent(event) {
    ...
    if (!file.name.endsWith('.zip')) {
      showNotification('error', '__TRANSLATE_error_select_zip_file__');
      return;
    }
    ...
    try {
      ...
      showNotification('error', `__TRANSLATE_error_importing_agent__: ${error.message}`);
    }
  }
    ...
    return html`
      <div class=\"agent-selector\">
        <select @change=${this.handleAgentChange} 
          ...
          <option value=\"\">__TRANSLATE_select_an_agent__</option>
      ...
      <form class=\"agent-form ${!this.agent ? 'form-hidden-when-no-agent' : ''}\" @submit=${this.handleSubmit}>
          <div class=\"form-group\">
            <label class=\"required\">__TRANSLATE_label_name_required__</label>
          ...
          <div class=\"form-group\">
            <label>__TRANSLATE_label_description__</label>
          ...
          <div class=\"form-group\">
            <label class=\"required\">__TRANSLATE_label_persona_required__</label>
            <select name=\"persona\" 
                    .value=${agentForRender.persona || ''}
                    @change=${this.handleInputChange}>
              <option value=\"\">__TRANSLATE_select_a_persona__</option>
          ...
          <div class=\"form-group\">
            <div class=\"form-group-header\">
              <label class=\"required\">__TRANSLATE_label_instructions_required__</label>
          ...
          <div class=\"form-group\">
            <div class=\"form-group-header\">
              <label>__TRANSLATE_label_technical_instructions__</label>
              <div class=\"form-group-actions\">
          ...
          <div class=\"form-group\">
            <label>
              __TRANSLATE_label_uncensored__
              <toggle-switch 
          ...
          <div class=\"form-group commands-section\">
            <details>
              <summary>__TRANSLATE_summary_preferred_providers__</summary>
          ...
          <div class=\"form-group commands-section\">
            <details>
              <summary>__TRANSLATE_summary_select_models__</summary>
          ...
          <div class=\"form-group commands-section\">
            <details>
              <summary>__TRANSLATE_summary_recommended_plugins__</summary>
          ...
          ${this.agent?.name && this.missingCommands && Object.keys(this.missingCommands).length > 0 ? html`
            <div class=\"form-group commands-section\">
              <details>
                <summary>__TRANSLATE_summary_missing_commands__ (${Object.keys(this.missingCommands).length})</summary>
          ...
          <div class=\"form-group commands-section\">
            <details><summary>__TRANSLATE_summary_available_commands__</summary>
            ${this.renderCommands()}
            </details>
          </div>
          ...
          <button class=\"btn\" type=\"submit\" ?disabled=${this.loading}>
            ${this.loading ? '__TRANSLATE_button_saving__' : '__TRANSLATE_button_save__'}
          </button>
      </form>

      ${!this.agent ? html`
        <div class=\"no-agent-message\">
          __TRANSLATE_info_no_agent_selected__
        </div>
      ` : ''}
    `;
   }
