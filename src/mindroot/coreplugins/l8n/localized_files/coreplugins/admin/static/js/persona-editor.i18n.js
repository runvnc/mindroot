...
class PersonaEditor extends BaseEl {
  ...
  async handleSubmit(event) {
    ...
    if (response.ok) {
      alert('__TRANSLATE_alert_persona_saved_successfully__');
      this.newPersona = false;
      this.fetchPersonas();
    } else {
      alert('__TRANSLATE_alert_failed_to_save_persona__');
    }
  }
      ...
      <div class="persona-editor">
        <div class="persona-selector">
          <div class="scope-selector">
            <label>
              <input type="radio" name="scope" value="local" .checked=${this.scope === 'local'} @change=${this.handleScopeChange} /> __TRANSLATE_scope_local__
            </label>
            <label>
              <input type="radio" name="scope" value="shared" .checked=${this.scope === 'shared'} @change=${this.handleScopeChange} /> __TRANSLATE_scope_shared__
            </label>
            <label>
              <input type="radio" name="scope" value="registry" .checked=${this.scope === 'registry'} @change=${this.handleScopeChange} /> __TRANSLATE_scope_registry__
            </label>
          </div>
          ...
          <button class="btn btn-secondary" @click=${this.handleNewPersona}>__TRANSLATE_button_new_persona__</button>
        </div>
        <form @submit=${this.handleSubmit} class="persona">
          <div class="form-group">
            <label>__TRANSLATE_label_name__</label>
        ...
        <div class="file-upload-container">
          <label class="file-upload-label" for="faceref">__TRANSLATE_label_choose_face_reference_image__</label>
          <input id="faceref" type="file" name="faceref" @change=${this.handleFileChange} />
          <span class="file-name">${this.facerefFileName}</span>
        </div>
        <div class="file-upload-container">
          <label class="file-upload-label" for="avatar">__TRANSLATE_label_choose_avatar_image__</label>
          <input id="avatar" type="file" name="avatar" @change=${this.handleFileChange} />
          <span class="file-name">${this.avatarFileName}</span>
        </div>
        <button class="btn" type="submit" id="save-persona">__TRANSLATE_button_save_persona__</button>
      </form>
      </div>
    `;
  }
