...
async def handle_reset_password(request: Request, token: str, password: str = Form(...), confirm_password: str = Form(...), services: ServiceProvider = Depends(ServiceProvider)):
...
async def admin_initiate_reset(username: str, services: ServiceProvider = Depends(ServiceProvider)):
    try:
